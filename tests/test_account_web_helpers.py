...
class TestWebAppSource(unittest.TestCase):
    def test_web_app_exists_and_has_ui(self):
        ...
        self.assertIn("/api/feed/proxies", src)
        self.assertIn("btnDeleteInvalid", src)
        self.assertIn("deleteInvalidAccounts", src)
        self.assertNotIn("/api/proxies/upload-resin", src)
        self.assertNotIn("btnUploadResin", src)
        self.assertNotIn("uploadAllToResin", src)
